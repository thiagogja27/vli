...
export function parseNFE(xmlString: string): NFEData {
  ...
  const terminalEntrega = extractTerminalEntrega(infAdic)
  const transbordo = extractTransbordo(infAdic)
  const retirada = extractRetirada(infAdic)
...
function extractTerminalEntrega(infComplementares: string): string {
  ...
  // 1. Tenta padrões específicos como "TERMINAL DE ENTREGA: TEG"
  match = infComplementares.match(/TERMINAL DE ENTREGA: (TEG|TEAG)/i);
  if (match && match[1]) {
    return match[1];
  }

  // 2. Tenta padrão genérico "ENTREGA:"
  match = infComplementares.match(/ENTREGA:\s*([^,;]+)/i);
  if (match && match[1]) {
    return match[1].trim();
  }
  
  // 3. Tenta os padrões "ALFANDEGADO" ou "ALFADEGADO"
...
function extractTransbordo(infComplementares: string): string {
  ...
  // Padrão 1: Procura por "TRANSBORDO EM:" ou "TRANSBORDO:", capturando até o ponto e vírgula.
  let match = infComplementares.match(/TRANSBORDO(?:\s*EM)?:\s*([^;]+)/i);
  if (match && match[1]) {
    return match[1].trim();
  }

  // Padrão 2: Procura por "CIDADE DE TRANSBORDO:"
  match = infComplementares.match(/CIDADE DE TRANSBORDO:\s*([^;]+)/i);
  if (match && match[1]) {
    return match[1].trim();
  }
...
function extractRetirada(infComplementares: string): string {
  if (!infComplementares) return ""
  // Procura por "RETIRADA EM:" ou "RETIRADA:", capturando até o ponto e vírgula.
  const match = infComplementares.match(/RETIRADA(?:\s*EM)?:\s*([^;]+)/i)
  if (match && match[1]) {
    return match[1].trim()
  }
  return ""
